/**
 * Mock Customer Data for MVP Demo
 *
 * Replace with real API calls in Phase 2.
 * See agent-specs/mock-data/mockCustomers.ts for original data.
 */

import { CustomerStats, LegacyCustomer } from "../types";

export const mockCustomers: LegacyCustomer[] = [
  {
    id: 1,
    name: "Rahul Sharma",
 ...
 */
export const getCustomerStats = (customers: LegacyCustomer[]): CustomerStats => {
  return {
 ...
 */
export const getRiskExplanation = (customer: LegacyCustomer): string => {
  ...
  if (customer.risk_level === "MEDIUM") {
    return `${customer.name} needs attention. Their usage has ${customer.usage_trend} and they are experiencing: ${formatIssueType(customer.issue_type)}. While not critical, addressing this issue promptly can prevent further disengagement.`;
  }
